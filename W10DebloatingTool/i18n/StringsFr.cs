
using System;

namespace W10DebloatingTool.i18n
{
    public class StringsFr : IStrings
    {
        public string LetsGo
        {
            get { return "Allons-y !"; }
        }

        public string SaveLogs
        {
            get { return "Enregistrer les Logs"; }
        }

        public string ChooseLogsPath
        {
            get { return "Veuillez choisir où sauvegarder les logs d'éxécution"; }
        }

        public string Privacy
        {
            get { return "Confidentialité"; }
        }

        public string Applications
        {
            get { return "Applications"; }
        }

        public string RemoveCortana
        {
            get
            {
                return "Supprimer Cortana";
            }
        }

        public string RemoveOneDrive
        {
            get
            {
                return "Supprimer OneDrive";
            }
        }

        public string Remove3DBuilder
        {
            get
            {
                return "Supprimer 3D Builder";
            }
        }

        public string RemoveGetStarted
        {
            get
            {
                return "Supprimer Prise en Main";
            }
        }

        public string RemoveOfficeHub
        {
            get
            {
                return "Supprimer Hub Microsoft Office";
            }
        }

        public string RemoveSolitaire
        {
            get
            {
                return "Supprimer Solitaire";
            }
        }

        public string RemoveSkype
        {
            get
            {
                return "Supprimer Skype";
            }
        }

        public string RemoveMaps
        {
            get
            {
                return "Supprimer Cartes";
            }
        }

        public string RemoveOneNote
        {
            get
            {
                return "Supprimer OneNote";
            }
        }

        public string RemoveWeather
        {
            get
            {
                return "Supprimer Bing Météo";
            }
        }

        public string RemoveSport
        {
            get
            {
                return "Supprimer Bing Sport";
            }
        }

        public string RemoveFinance
        {
            get
            {
                return "Supprimer Bing Finance";
            }
        }

        public string RemoveNews
        {
            get
            {
                return "Supprimer Bing Actualité";
            }
        }

        public string RemoveXbox
        {
            get
            {
                return "Supprimer Xbox";
            }
        }

        public string RemoveMusic
        {
            get
            {
                return "Supprimer Groove Musique";
            }
        }

        public string RemoveVideo
        {
            get
            {
                return "Supprimer Films";
            }
        }

        public string RemovePhotos
        {
            get
            {
                return "Supprimer Photos";
            }
        }

        public string RemoveCamera
        {
            get
            {
                return "Supprimer Caméra";
            }
        }

        public string RemoveAlarms
        {
            get
            {
                return "Supprimer Alarmes";
            }
        }

        public string RemoveContacts
        {
            get
            {
                return "Supprimer Contacts";
            }
        }

        public string RemoveMail
        {
            get
            {
                return "Supprimer Courrier et Calendrier";
            }
        }

        public string RemovePhoneCompanion
        {
            get
            {
                return "Supprimer Assitant mobile";
            }
        }

        public string RemoveSoundRecord
        {
            get
            {
                return "Supprimer Enregistreur vocal";
            }
        }

        public string RemoveEdge
        {
            get
            {
                return "Supprimer Microsoft Edge";
            }
        }

        public string RemoveAllMetroApps
        {
            get
            {
                return "Supprimer TOUTES les applis Metro";
            }
        }

        public string SelectAllApps
        {
            get
            {
                return "Cocher toutes les applis";
            }
        }

        public string UnselectAllApps
        {
            get
            {
                return "Décocher toutes les applis";
            }
        }

        public string Services
        {
            get
            {
                return "Services";
            }
        }

        public string Ergonomy
        {
            get
            {
                return "Ergonomie";
            }
        }

        public string NoLockscreen
        {
            get
            {
                return "Désactiver l'écran de verrouillage";
            }
        }

        public string NotWindows10Error
        {
            get { return "Ce programme est conçu pour être executé sur Windows 10."; }
        }

        public string Informations
        {
            get
            {
                return "Informations";
            }
        }

        public string Version
        {
            get
            {
                return "Version {0}";
            }
        }

        public string Close
        {
            get
            {
                return "Fermer";
            }
        }

        public string UseAdsId
        {
            get {
                return "Utilise mon identifiant pour les publicités";
            }
        }

        public string AllowSitesToAccessMyLanguage
        {
            get
            {
                return "Autoriser les sites à détecter à ma langue";
            }
        }

        public string DisableSmartScreen
        {
            get
            {
                return "Désactiver SmartScreen";
            }
        }

        public string DisablePhoneServices
        {
            get
            {
                return "Désactiver les services téléphoniques";
            }
        }

        public string DisableBiometricSystem
        {
            get
            {
                return "Désactiver le système biométrique";
            }
        }

        public string DisableDiagTrack
        {
            get
            {
                return "Désactiver DiagTrack";
            }
        }

        public string DisableAutoLogger
        {
            get
            {
                return "Désactiver AutoLogger";
            }
        }

        public string DisableDmwAppPushService
        {
            get
            {
                return "Désactiver DmwAppPushService";
            }
        }

        public string BlockTrackingServers
        {
            get
            {
                return "Désactiver les serveurs de traçage";
            }
        }

        public string NoTilesOnStartMenu
        {
            get
            {
                return "Désactiver les tuiles dans le Menu Démarrer";
            }
        }

        public string SearchBar
        {
            get
            {
                return "La barre de recherche dans la barre des tâches :";
            }
        }

        public string DisableIt
        {
            get { return "La Désactiver"; }
        }

        public string AsButton
        {
            get { return "Comme bouton"; }
        }

        public string AsBar
        {
            get { return "Comme barre"; }
        }

        public string EnableGodMode
        {
            get { return "Activer le Mode Dieu"; }
        }

        public string NothingToDo
        {
            get
            {
                return "Rien à faire !\n\nVeuillez cocher au moins une option.";
            }
        }

        public string RestartComputer
        {
            get
            {
                return "Voilà c'est terminé !\nIl faut maintenant redémarrer votre ordinateur pour finaliser les modifications.\nEnregistrez tous vos documents et cliquez sur OK pour redémarrer.";
            }
        }

        public string ProcessingWillTakeAWhile
        {
            get
            {
                return
                    "Le programme va maintenant appliquer les modifications.\nCette opération peut prendre du temps et l'application va se figer le temps de tout faire.";
            }
        }

        public string BlockLocalization
        {
            get
            {
                return "Empêcher l'accès à la localisation";
            }
        }

        public string BlockDevices
        {
            get
            {
                return "Empêcher l'accès aux périphériques";
            }
        }

        public string PreventCortana
        {
            get
            {
                return "Empêcher Cortana de vous connaître";
            }
        }

        public string PreventSync
        {
            get
            {
                return "Empêcher la synchronisation des appareils";
            }
        }

        public string DisableDefender
        {
            get
            {
                return "Désactiver Windows Defender";
            }
        }

        public string DisableCloudProtection
        {
            get
            {
                return "Désactiver la Protection dans le Cloud";
            }
        }

        public string DisableSamples
        {
            get
            {
                return "Désactiver l'envoi d'échantillons";
            }
        }

        public string DisableAutoUpdate
        {
            get
            {
                return "Désactiver les mises à jour automatiques";
            }
        }

        public string AllowDifferedUpdate
        {
            get
            {
                return "Activer les mises à jour différées";
            }
        }

        public string DisableUpdateSharing
        {
            get
            {
                return "Désactiver le partage des mises à jour";
            }
        }

        public string PreventKeyboard
        {
            get
            {
                return "Empêcher l'envoi de ce qui est tapé sur le clavier";
            }
        }

        public string CheckAll
        {
            get { return "Tout cocher"; }
        }

        public string UncheckAll
        {
            get { return "Tout décocher"; }
        }

        public string DisableAdvancedTracking
        {
            get
            {
                return "Désactiver le traçage avancé";
            }
        }

        public string AboutText
        {
            get
            {
                return "W10 Debloating Tool est une petite application dont le but est de vous affranchir de l’espionnage abusif de Microsoft sur Windows 10, mais en plus de cela il vous permettra de désinstaller les applications de base et rajoute quelques choix ergonomiques.\n\nMerci d’avoir téléchargé notre outil !\nN’hésitez pas à le partager, il est gratuit et open-source!";
            }
        }
    }
}
